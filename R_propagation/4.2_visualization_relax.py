...
# load rans+ml data
U_rans_ml = np.load('data/U_rans_ml_relax.npy')[:int(n_cells/2), 0]
k_rans_ml = np.load('data/k_rans_ml_relax.npy')[:int(n_cells/2)] / (u_tau_ml ** 2)
omega_rans_ml = np.load('data/omega_rans_ml_relax.npy')[:int(n_cells/2)]

# load rans+dns data
U_rans_dns = np.load('data/U_rans_dns_relax.npy')[:int(n_cells/2), 0]
k_rans_dns = np.load('data/k_rans_dns_relax.npy')[:int(n_cells/2)] / (u_tau_ml ** 2)
omega_rans_dns = np.load('data/omega_rans_dns_relax.npy')[:int(n_cells/2)]
...
plt.savefig(f'plots/mean_velocity_relax.png', bbox_inches='tight')
...
plt.grid(color='grey', linestyle='--', linewidth=0.5)
plt.savefig(f'plots/turbulent_kinetic_energy_relax.png', bbox_inches='tight')
...
plt.ylabel('U+')
plt.grid(color='grey', linestyle='--', linewidth=0.5)
plt.savefig(f'plots/mean_velocity_yplus_relax.png', bbox_inches='tight')
...
plt.legend(loc='lower right')
plt.grid(color='grey', linestyle='--', linewidth=0.5)
plt.savefig(f'plots/turbulent_kinetic_energy_yplus_relax.png', bbox_inches='tight')
