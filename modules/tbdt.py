...
class TBDT:
    ...
    def createSplit(self, X, Y, TB, TT_mat, Tf_mat):
        ...
        def findJMin_sorted(i1, X, Y, TB, TT_mat, Tf_mat):
            ...
            if X_sorted.shape[1] == 2:
                g_l, _, diff_l = self.fitTensor(TT_sorted[:, :, :1], Tf_sorted[:, :1], TB_sorted[:, :, :1],
                                                Y_sorted[:, :1])
                g_r, _, diff_r = self.fitTensor(TT_sorted[:, :, 1:], Tf_sorted[:, 1:], TB_sorted[:, :, 1:],
                                                Y_sorted[:, 1:])
            ...
            # splitting of the branch in self.fit()
                ...
                for i2 in range(1, X_sorted.shape[1] - 1):
                    ...
                    g_l, _, diff_l = self.fitTensor(TT_sorted[:, :, :i2], Tf_sorted[:, :i2], TB_sorted[:, :, :i2],
                                                    Y_sorted[:, :i2])
                    g_r, _, diff_r = self.fitTensor(TT_sorted[:, :, i2:], Tf_sorted[:, i2:], TB_sorted[:, :, i2:],
                                                    Y_sorted[:, i2:])
                    diff = np.hstack([diff_l, diff_r])
                    J_tmp = (np.mean(np.square(diff)))
        ...
        def findJMin_opt(i1, X, Y, TB, TT_mat, Tf_mat):
            ...
            def objfn_J(ifloat, Y_sorted, TB_sorted, TT_sorted, Tf_sorted):
                ...
                i = int(ifloat)
                g_l, _, diff_l = self.fitTensor(TT_sorted[:, :, :i], Tf_sorted[:, :i], TB_sorted[:, :, :i],
                                                Y_sorted[:, :i])
                g_r, _, diff_r = self.fitTensor(TT_sorted[:, :, i:], Tf_sorted[:, i:], TB_sorted[:, :, i:],
                                                Y_sorted[:, i:])
                diff = np.hstack([diff_l, diff_r])
            ...
            g_l, _, diff_l = self.fitTensor(TT_sorted[:, :, :i_split], Tf_sorted[:, :i_split],
                                            TB_sorted[:, :, :i_split], Y_sorted[:, :i_split])
            g_r, _, diff_r = self.fitTensor(TT_sorted[:, :, i_split:], Tf_sorted[:, i_split:],
                                            TB_sorted[:, :, i_split:], Y_sorted[:, i_split:])
            i_left_sorted = np.zeros(X_sorted.shape[1], dtype=bool)
        ...
        if self.splitting_features == 'all':  # use all available features for each split
            ...
            randomFeat = np.random.choice(np.linspace(0, X.shape[0] - 1, X.shape[0], dtype=int), size=n_feat,
                                          replace=False)
            X = X[randomFeat, :]
    ...
    def predict(self, X, TB, tree):
        ...
        X = X.T
        TB = TB.T
        TB = TB.reshape((9, TB.shape[2], TB.shape[3]))
    ...
    def fit(self, X, Y, TB):
        ...
        for i in range(levels_max):
            ...
            if i == 0:  # TODO: merge creation of root node (i=0) and further nodes
                ...
                splitVar, splitVal, i_left, i_right, g_l, g_r, MSE_l, MSE_r, n_l, n_r = self.createSplit(X, Y, TB,
                                                                                                         TT_mat, Tf_mat)
                ...
                for i1 in range(len(Queue)):
                    ...
                    splitVar, splitVal, i_left, i_right, g_l, g_r, MSE_l, MSE_r, n_l, n_r = self.createSplit(
                        X[:, indices], Y[:, indices],
