...
# read prepared data
b_RF = np.load(f'../b_prediction_old/data/predicted_b_TBRF_filtered.npy')
b_RF_mirrored = np.flip(b_RF, axis=0)
...
b_interpolant = interp1d(cell_centers, b_RF_extended, axis=0, kind='cubic')
b_RF_filtered_intd = b_interpolant(rans_cell_centers)

# prepare and save b_ij field in OpenFOAM format
b_ij_field = ''
for i in range(2):
    for j in range(int(b_RF_filtered_intd.shape[0]/2)):
        x = b_RF_filtered_intd[j]
        b_ij_field += f'({x[0, 0]} {x[0, 1]} {x[0, 2]} {x[1, 1]} {x[1, 2]} {x[2, 2]})\n'
        b_ij_field += f'({x[0, 0]} {x[0, 1]} {x[0, 2]} {x[1, 1]} {x[1, 2]} {x[2, 2]})\n'

for i in range(2):
    for j in range(int(b_RF_filtered_intd.shape[0]/2), int(b_RF_filtered_intd.shape[0])):
        x = b_RF_filtered_intd[j]
        b_ij_field += f'({x[0, 0]} {x[0, 1]} {x[0, 2]} {x[1, 1]} {x[1, 2]} {x[2, 2]})\n'
        b_ij_field += f'({x[0, 0]} {x[0, 1]} {x[0, 2]} {x[1, 1]} {x[1, 2]} {x[2, 2]})\n'
