...
import numpy as np
import pandas as pd
from modules.utils_old import generate_b, prepare_R_RANS, prepare_R


R_GB = np.load(f'data/predicted_R_TBGB_filtered.npy')
R_RF = np.load(f'data/predicted_R_TBRF_filtered.npy')
...
for n, (i, j) in enumerate(((0, 0), (1, 1), (2, 2), (0, 1))):
    ...
    axs[n].set_title(f"b_{i+1}{j+1} u'v' u'u' v'v'")
...
# plt.xscale('log')
plt.savefig(f'plots/ML_R_filtered_logx.png', bbox_inches='tight')